...
export class HistoryController {
  ...
  getOrderHistory = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.historyService.getOrderHistory(
        userId,
        activeRole,
        validatedQuery,
      );
  ...
  getOrderDetailByToCode = async (req: Request, res: Response) => {
    try {
      // Validasi Parameter Param URL
      const parseResult = getOrderDetailParamSchema.safeParse(req.params);
      if (!parseResult.success) {
        return res.status(400).json({
          success: false,
          // 🟢 Ganti 'errors' menjadi 'issues'
          message:
            parseResult.error.issues[0]?.message || "Parameter tidak valid.",
        });
      }

      const { toCode } = parseResult.data;
      const data = await this.historyService.getOrderDetailByToCode(toCode);

      return res.status(200).json({
        success: true,
        message: "Berhasil mengambil detail Travel Order",
        data,
      });
    } catch (error: any) {
